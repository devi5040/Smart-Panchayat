...
describe("Category Services", () => {
  ...
  describe("getSignedUrl", () => {
    ...
    it("should throw an error if S3 interaction fails", async () => {
      ...
      await expect(getSignedUrl("test.jpg", "image/jpeg")).rejects.toThrow(
        "S3 error"
      );
    });
  });
  ...
  describe("addCategory", () => {
    ...
    it("should throw an error if database interaction fails", async () => {
      ...
      await expect(addCategory("Category 4", "test-url")).rejects.toThrow(
        "Database error"
      );
    });
  });
  ...
  describe("updateCategory", () => {
    ...
    it("should throw an error if categoryId is invalid", async () => {
      await expect(
        updateCategory(null, "Updated Category", "updated-url")
      ).rejects.toThrow("Category ID is not valid");
      await expect(
        updateCategory(0, "Updated Category", "updated-url")
      ).rejects.toThrow("Category ID is not valid");
      await expect(
        updateCategory("abc", "Updated Category", "updated-url")
      ).rejects.toThrow("Category ID is not valid");
    });
    ...
    it("should throw an error if no rows were updated", async () => {
      ...
      await expect(
        updateCategory(1, "Updated Category", "updated-url")
      ).rejects.toThrow("Category ID is not valid. No records updated.");
    });
  ...
  describe("deleteCategory", () => {
    ...
    it("should throw an error if categoryId is invalid", async () => {
      await expect(deleteCategory(null)).rejects.toThrow(
        "Category ID is not valid"
      );
      await expect(deleteCategory(0)).rejects.toThrow(
        "Category ID is not valid"
      );
      await expect(deleteCategory("abc")).rejects.toThrow(
        "Category ID is not valid"
      );
    });
