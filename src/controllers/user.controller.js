...
exports.addUser = async (req, res) => {
  const { name, mobileNumber, languagePreference, latitude, longitude, role } = req.body;

  try {
    const user = await userServices.getUserByMobileNumber(mobileNumber);
    if (user !== null)
      return res.status(400).json({ message: 'User already exists.', userExists: true });
    ...
    res.status(500).json({ message: 'Internal error while adding user.', error });
  }
};
...
exports.updateLanguagePreferrence = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'The system language has been set successfully.' });
  } catch (error) {
    logger.error(
      `Internal error while updating preferred language.
        ${error}`,
    );
    res.status(500).json({ message: 'Internal error while updating the user details.' });
  }
};
...
exports.changeUserRole = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Internal error while changing user role.' });
  }
};
...
exports.addPassword = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Added password to the user successfully.' });
  } catch (error) {
